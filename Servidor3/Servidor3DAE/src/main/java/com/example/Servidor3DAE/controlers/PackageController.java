...
public class PackageController {
    ...
    @GetMapping("/get/")
    public ResponseEntity<List<PaqueteDTO>> getAllPackages() {
        List<CulturalPackage> packages = packageService.getList();
        List<PaqueteDTO> xxxx = new ArrayList<>();
        for (CulturalPackage c: packages) {
            List<Integer> xdd = new ArrayList<>();
                for(Guide xxx : c.getGuias())
                {
                    xdd.add(xxx.getId());
                }
                PaqueteDTO xxx = new PaqueteDTO(c.getNombre(),
                        c.getId(),
                        c.getPrecio(),
                        c.getFechaInicio(),
                        c.getFechaFin(),
            ...
            return ResponseEntity.ok(xxxx); // 200 OK// 200 OK
    }
